 ...
 * Auth: X-Partner-Api-Key + Bearer access_token (login email/pass).
 */
const fs = require("fs");
...
function readPartnerConfig(configFile) {
  ...
  return {
    ...
    /** Base URL cho API Đơn Admin (nếu prod chưa deploy — dùng Basso local) */
    adminBaseUrl: String(process.env.BASSO_ADMIN_BASE_URL || p.adminBaseUrl || "").replace(/\/$/, ""),
...
function partnerConfigured(cfg) {
  return !!(cfg.enabled && cfg.baseUrl && cfg.apiKey && cfg.email && cfg.password);
}
...
async function withToken(cfg, fn) {
  let auth = await login(cfg);
  try {
    return await fn(auth.accessToken);
  } catch (err) {
    const msg = String(err.message || err);
    if (/access token|Unauthorized|401/i.test(msg)) {
      auth = await login(cfg, { force: true });
      return await fn(auth.accessToken);
    }
    throw err;
  }
...
async function getWebOrderCreateMeta(cfg) {
...
async function getWebOrderCreateListPage(cfg, { tab = "order", page = 1, pageSize } = {}) {
  return withToken(cfg, async (token) => {
...
async function getWebOrderCreateListAll(cfg, tab = "order") {
  const first = await getWebOrderCreateListPage(cfg, { tab, page: 1 });
  const orders = [...(first.orders || [])];
  const items = [...(first.items || [])];
  const totalPage = Number((first.pagination && first.pagination.total_page) || 1);
  for (let page = 2; page <= totalPage; page++) {
    const next = await getWebOrderCreateListPage(cfg, { tab, page });
...
async function getLastWebPaymentByWebsite(cfg, { days = 180, websites = [] } = {}) {
  ...
  for (const base of tryBases) {
    ...
    try {
      const data = await withToken(localCfg, async (token) => {
        const q = new URLSearchParams();
 ...
 */
async function createWebOrder(cfg, payload) {
  const tryBases = [cfg.baseUrl];
  if (cfg.adminBaseUrl && cfg.adminBaseUrl !== cfg.baseUrl) {
    tryBases.push(cfg.adminBaseUrl);
  }
  ...
  for (const base of tryBases) {
    const localCfg = { ...cfg, baseUrl: base };
    try {
      return await withToken(localCfg, async (token) => {
...
function mapOrder(raw, status, itemsByOrderId, picList, overlay) {
  ...
  return {
    ...
    website: raw.website || "",
    rate: raw.currency_rate != null && raw.currency_rate !== "" ? `${raw.currency_rate}đ` : "",
 ...
 */
async function fetchAllWebOrders(cfg, { picList = [], overlay = {} } = {}) {
  const tabs = ["order", "checkout", "need_handle"];
  const all = [];
  const pendingByTab = {};
  const websiteSet = new Set();
  for (const tab of tabs) {
    const data = await getWebOrderCreateListAll(cfg, tab);
