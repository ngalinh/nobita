...
function readPartnerConfig(configFile) {
  ...
  const rawEmail = String(process.env.BASSO_EMAIL || p.email || "").trim();
  const rawPass = String(process.env.BASSO_PASS || p.password || "").trim();
  const email =
    !rawEmail || rawEmail === "-" || /^none$/i.test(rawEmail) || /your@email/i.test(rawEmail)
      ? ""
      : rawEmail;
  const password =
    !rawPass || rawPass === "-" || /^none$/i.test(rawPass) || /your-password/i.test(rawPass)
      ? ""
      : rawPass;
  return {
